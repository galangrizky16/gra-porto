...
export async function PUT(
  ...
  try {
    ...
    console.error("[PUT pendidikan] unexpected:", err);
...
export async function DELETE(
  ...
  try {
    ...
    const { error } = await db.from("tentang_pendidikan").delete().eq("id", id);
    ...
    console.error("[DELETE pendidikan] unexpected:", err);
    return NextResponse.json({ error: String(err) }, { status: 500 });
  }
}